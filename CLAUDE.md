...
| `src/client/components/*.tsx` (`ListView`, `ListPicker`, `ListSidebar`, `RosterRows`, `AccountButton`, `ConfirmDialog`, `LanguageDialog`, `focus.ts`, `media.ts`) — roster UI, account/header controls, dialog focus contract, responsive tiers | explanation | [docs/explanation/architecture.md](docs/explanation/architecture.md) (Design & UX, components) |
