...
| `src/client/store/sync.ts`, `syncStatus.ts`, `identity.ts`, `teardown.ts`, `StoreProvider.tsx`, `components/SyncStatus.tsx`, `components/SyncNotice.tsx` | explanation | [docs/explanation/auth-and-sync.md](docs/explanation/auth-and-sync.md) (reconnect, sync status surfaces, offline identity, teardown) |
...
| `src/client/components/*.tsx` (`ListView`, `ListPanel`, `ListSheetWrapper`, `ListSidebarWrapper`, `ListRows`, `AccountButton`, `ConfirmDialog`, `LanguageDialog`, `focus.ts`, `media.ts`) — lists UI, account/header controls, dialog focus contract, responsive tiers | explanation | [docs/explanation/architecture.md](docs/explanation/architecture.md) (Design & UX, components) |
...
| `csp/**` — a policy source added, removed, or scoped to dev/preview only | explanation; how-to | [docs/explanation/architecture.md](docs/explanation/architecture.md) (Content-Security-Policy row); [docs/how-to/deploy.md](docs/how-to/deploy.md) (CSP section); [docs/how-to/check-on-a-device.md](docs/how-to/check-on-a-device.md) |
