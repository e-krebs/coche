...
oxfmt can sort Tailwind classes; it is left at its default of not doing so, and Tailwind class
wrapping *and* ordering both live in oxlint instead, via the `readable-tailwind` jsPlugin. The
...
The same boundary explains why the repo carries an [`../../.editorconfig`](../../.editorconfig) at
all. oxfmt's `ignorePatterns` excludes Markdown, CSS, HTML and `package.json`, so for those files
nothing in the toolchain settles indentation, line endings or a trailing newline — the editor does,
or nobody does. `.editorconfig` states them once for every file type in a format editors read
natively, and for `.ts`/`.tsx` it restates oxfmt's own defaults so the two can never disagree. The
100-column ruler is deliberately *not* there: the property that would express it is unimplemented by
the VS Code EditorConfig extension, so it lives in [`../../.vscode/settings.json`](../../.vscode/settings.json)
where it renders. That file also pins oxfmt as the default formatter for TypeScript, which matters
more than it sounds — a contributor arriving with Prettier installed and format-on-save enabled will
otherwise have it undo the wrap-then-sort shape `readable-tailwind` just produced, and
[`../../.vscode/extensions.json`](../../.vscode/extensions.json) can only decline to *recommend*
Prettier, never disable one already installed.
...
One file is excluded from linting outright rather than tiered: `src/client/routeTree.gen.ts` is
generated by the router plugin and committed, so a diagnostic on it would not be actionable — the
fix would have to land in a generator, and the next build would overwrite it anyway. The exclusion
is pre-emptive rather than a response to anything: the file lints clean today, and naming it in
`ignorePatterns` in [`../../.oxlintrc.json`](../../.oxlintrc.json) as well as
[`../../.oxfmtrc.json`](../../.oxfmtrc.json) — where it already was, alongside the editing hook that
skips it — means a change in the generator's output can't turn a required check red on code nobody
wrote.
...
- **`prefer-nullish-coalescing` sets `ignorePrimitives.boolean`.** The repo's conditional-styling
  idiom is `data-active={active || undefined}` — see the `styling` skill
  ([`../../.claude/skills/styling/SKILL.md`](../../.claude/skills/styling/SKILL.md)) — which depends
  on `||` treating `false` the same as
...
`yarn fix` convergence once they are all in;
[../adr/0019-grouped-dependency-updates.md](../adr/0019-grouped-dependency-updates.md) records that
