...
test.describe("sync-merge", () => {
  test("edits sync live between two contexts of the same user", async ({
    ...
    try {
      ...
      // Adding leaves focus in the add field, which aria-disables the row; blur and wait before
      // clicking.
      await field(page).blur();
      await expect(row(page, "Milk")).not.toHaveAttribute("aria-disabled", "true");
