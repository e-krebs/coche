  ...
  webServer: [
    {
      ...
      command: `wrangler dev --port ${WORKER_PORT} --var DEV:true --var ALLOWED_ORIGINS:${APP_ORIGIN} --var CLERK_AUTHORIZED_PARTIES:${APP_ORIGIN} --persist-to .wrangler/e2e-state`,
    ...
    {
      // Bare `vite preview` and `wrangler dev` above: nub does not forward termination to its
      // grandchild, so a wrapped server outlives Playwright's kill and the run never exits.
      command: `nub run build:e2e:sync && vite preview --port ${APP_PORT} --strictPort`,
      url: APP_ORIGIN,
      reuseExistingServer: !CI,
      timeout: 120_000,
    },
